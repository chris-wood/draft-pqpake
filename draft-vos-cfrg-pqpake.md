...
uniform, it uses Kemeleon {{!KEMELEON=I-D.veitch-kemeleon}} to (re-)encode ML-KEM's
public keys via `Kemeleon.EncodeEkR` and `Kemeleon.DecodeEkR`.
...
Because `Kemeleon.EncodeEkR` leaves the final 32 bytes (⍴) of the encoding untouched,
